...
elfgen_baseplot <- function(watershed.df,quantile,glo,ghi,method) {
  ...
  upper.quant.data <- rq(y_var ~ log(x_var),data = full_dataset, tau = quantile)
  newy <- c(log(full_dataset$x_var)*coef(upper.quant.data)[2]+coef(upper.quant.data)[1])
  upper.quant <- subset(full_dataset, full_dataset$y_var > newy)
  ...
  if(length(breaks) != 0) {
    ...
    #subset_n <- length(watershed.df$y_var)
    ...
    # print(head(data))
    # print(subset_n)
     print(paste("Breakpoint identified at",breakpt,colnames(watershed.df.raw)[4],sep=" "))
