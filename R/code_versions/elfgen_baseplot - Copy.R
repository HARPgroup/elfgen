elfgen_baseplot <- function(watershed.df,quantile,ghi) {
  ...
  # QUANTREG SPECIFIC
  ######################################################################################
  data <- watershed.df[!(watershed.df$MAF > (ghi)),]
  
  #------------------------------
  # UPPER QUANTILE SUBSET
  upper.quant.data <- rq(NT.TOTAL.UNIQUE ~ log(MAF),data = data, tau = quantile)
  newy <- c(log(data$MAF)*coef(upper.quant.data)[2]+coef(upper.quant.data)[1])   
  upper.quant <- subset(data, data$NT.TOTAL.UNIQUE > newy)
  #------------------------------
  
  regupper <- lm(NT.TOTAL.UNIQUE ~ log(MAF),data = upper.quant) 
  ...
  rucount <- length(upper.quant$NT.TOTAL.UNIQUE)
  ...
  flow_title <- "MAF"
  biometric_title <- "NT.TOTAL.UNIQUE"
  
  yaxis_thresh <- max(full_dataset$NT.TOTAL.UNIQUE)
  ...
  result <- ggplot(watershed.df, aes(x=MAF,y=NT.TOTAL.UNIQUE)) + 
    ylim(0,yaxis_thresh) + 
    
     geom_point(data = full_dataset,aes(colour="aliceblue")) +
     geom_point(data = data,aes(colour="blue")) + 
     stat_smooth(method = "lm",fullrange=FALSE,level = .95, data = upper.quant, aes(x=MAF,y=NT.TOTAL.UNIQUE,color = "red")) +
     geom_point(data = upper.quant, aes(x=MAF,y=NT.TOTAL.UNIQUE,color = "black")) + 
     ...
     geom_smooth(data = upper.quant, formula = y ~ x, method = "lm", show.legend = TRUE, aes(x=MAF,y=NT.TOTAL.UNIQUE,color = "green"),se=FALSE) + 
    # 
