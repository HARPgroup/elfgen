...
elfgen <- function(watershed.df,quantile,breakpt,yaxis_thresh,xlabel = FALSE,ylabel = FALSE) {
  ...
  result <- ggplot(watershed.df, aes_string(x='x_var',y='y_var')) +
    ylim(0,yaxis_thresh) +
     ...
     geom_point(data = full_dataset,aes_string(x='x_var',y='y_var', colour="aliceblue")) +
     geom_point(data = data,aes_string(x='x_var',y='y_var', colour="blue")) +
     stat_smooth(method = "lm",fullrange=FALSE,level = .95, data = upper.quant, aes_string(x='x_var',y='y_var',color = "red")) +
     geom_point(data = upper.quant, aes_string(x='x_var',y='y_var',color = "black")) +
     geom_quantile(data = data, quantiles= quantile,show.legend = TRUE,aes_string(x='x_var',y='y_var', color="red")) +
     geom_smooth(data = data, method="lm",formula=y ~ x,show.legend = TRUE, aes_string(x='x_var',y='y_var', colour="yellow"),se=FALSE) +
     geom_smooth(data = upper.quant, formula = y ~ x, method = "lm", show.legend = TRUE, aes_string(x='x_var',y='y_var',color = "green"),se=FALSE) +
